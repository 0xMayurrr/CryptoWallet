...
export default function ReceiveScreen({ navigation }: any) {
  const { walletAddress, network, isDarkMode } = useWallet();
  ...
  const handleShare = async () => {
    ...
    try {
      await Share.share({
        message: `My ${network} wallet address: ${walletAddress}`,
    ...
    <View style={[styles.container, { backgroundColor: T.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionBlock}>
          <Text style={[styles.sectionLabel, { color: T.textMuted }]}>Network</Text>
          <View style={[styles.networkBadge, { backgroundColor: T.surfaceLow, borderColor: T.border }]}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              <View style={[styles.networkIcon, { backgroundColor: T.primaryDark + '20' }]}>
                <MaterialIcons name="share" size={16} color={T.primaryDark} />
              </View>
              <View>
                <Text style={[styles.networkName, { color: T.text }]}>{network} Network</Text>
                <Text style={[styles.networkArrival, { color: T.textMuted }]}>Estimated arrival: 1–3 mins</Text>
              </View>
            </View>
          </View>
        </View>
